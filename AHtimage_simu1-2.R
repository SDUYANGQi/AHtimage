...
for(re in 1:nseed){
  ...
  # data generation
    # imaging data generation and FPCA implementation
    if(1){
      # true eigenvectors
    ...
    # additive hazard model data generation
    Z   = cbind(1, rbinom(n,3,.5), runif(n), eigen_score)
    C   = rbeta(n,1,2.5) # runif(n,0,0.7) # 
    X   = array()
    ...
    for(i in 1:n){
      ran[i] = runif(1)
      ft     = function(t){ (A1(t) + A2(t)*Z[i,2] + A3(t)*Z[i,3] + A4(t)*Z[i,4]
                             + A5(t)*Z[i,5]+ A6(t)*Z[i,6])*1 + log(ran[i])}
    ...
    # par(mfrow = c(2,2)); hist(C); hist(Ti); hist(X)
    
    data = data.frame(cbind(Z[,c(1,2,3)],xi),X,delta)
    Z1   = data[,c(1:s)]; X = data[,s+1]; delta = data[,s+2]
  
  # function for inference of Aalen's additive hazard model.
  ...
  write.table(cbind(Arec[re,,],Srec[re,,],int), 
              file = paste(newdir,"/extra-", re, ".txt" ,sep = "") )
}

cat(paste("The result is recorded in the new folder: \n", newdir, sep = ""))
cat(paste("Please Use read_simu1-2.R for further result analysis"))
