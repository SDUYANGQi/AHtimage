...
AHtimage_simu1.R  + AHtimage_simu1_read.R (result summary code)
...
3. The result can be analyzed by simu1_read.R, yielding the CP table and the estimated vs. true functional curves.
...
AHtimage_simu3.R  + AHtimage_simu3_read.R (result summary code)
1. The setting in this code (for extra-) is n = 600, censoring rate approximately 60%, repeated for 1000 times.
2. The result is recorded and written out as txt file for each repeat. 
3. The result can be analyzed by simu3_read.R, yielding the CP table and the estimated vs. true functional curves.
